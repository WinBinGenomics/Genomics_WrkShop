...
vcfs=(downsampled_ENP_GOC_19.vcf.gz downsampled_ENP_GOC_21.vcf.gz downsampled_ENP_GOC_21.vcf.gz)
vcf="Passing_biallelic_all_19-21.vcf.gz"

cd "${homedir}/vcfs/filtered/pixy"
...
for FILE in "${vcfs[@]}"; do
    ...
    ####### Filtering ####### ENP VCFs
    ENP_OUT=$(printf "Passingsites_bisnps_ENP_%02d.vcf.gz" "$i")

    #bcftools view -f PASS -m2 -M2 -v snps -q 0.05:minor -Oz -o "${homedir}/LD_analysis/ENP/vcfs/$ENP_OUT" "$ENP_VCF"
    bcftools view -f PASS -m2 -M2 -v snps -Oz -o "${homedir}/LD_analysis/ENP/vcfs/$ENP_OUT" "$ENP_VCF"
    if [[ $? -ne 0 ]]; then
    ...
    ######## Filtering ####### GOC VCFs
    GOC_OUT=$(printf "Passingsites_bisnps_GOC_%02d.vcf.gz" "$i")

    #bcftools view -f PASS -m2 -M2 -v snps -q 0.05:minor -Oz -o "${homedir}/LD_analysis/GOC/vcfs/$GOC_OUT" "$GOC_VCF"
    bcftools view -f PASS -m2 -M2 -v snps  -Oz -o "${homedir}/LD_analysis/GOC/vcfs/$GOC_OUT" "$GOC_VCF"
    if [[ $? -ne 0 ]]; then
